    ...
    def collide(
        ...
        for obs, mesh in zip(self.obstacles, self.coalMeshes):
            ...
            if colRes.isCollision():
                ...
                normal *= -1 
                print(f"Num Contacts: {colRes.numContacts()}")
                print(f"normal:{normal} normal:{(nearestPoint2 - nearestPoint1) / (np.linalg.norm(nearestPoint2 - nearestPoint1))}")
                colRes.clear()
