...
class TrajectoryGeneration:
    def __init__(self, map=None):
        ...
        self.opti.set_value(
            self.xf, np.array([8, 3, 0, 0, 0, 0, 0.707, 0, 0, 0.707, 0, 0, 0])
        )
    ...
    def setup_problem(self):
        ...
        for i in range(N - 1):
            ...
            self.opti.subject_to(
                vel[:, i + 1]
                == vel[:, i] + dt * (1 / m) * ca.mtimes(R_current.as_matrix(), F)
            )
            # Quaternion update
            self.opti.subject_to(
            ...
            self.opti.subject_to(
                ...
                + dt
                * ca.mtimes(
                    ca.inv(J), M - ca.cross(ang_vel[:, i], ca.mtimes(J, ang_vel[:, i]))
                )
            )
        ...
        for obs in self.map.obstacles:
            ...
            V_O = (
                obs.global_vertices()
            )  # vertices of the polytope obstacle (3 x num_vertices)
            ...
            for i in range(N):
                ...
                self.opti.subject_to(
                    nu[0, i] ** 2 >= ca.dot(xi[:, i], ca.mtimes(P_R_inv, xi[:, i]))
                )
