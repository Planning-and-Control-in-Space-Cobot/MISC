...
class BagReader:
    def __init__(
        ...
        self.topics = topics
        self.topic_types = {}  # Dictionary to store topic types based on provided topics
        ...
        for topic in topic_types:
            if topic.name in self.topics:
                self.topic_types[topic.name] = get_message(topic.type)  # Get the ROS message type for each topic
                self.msgs[topic.name] = []  # Initialize an empty list for messages
    ...
    def read_next_msg(
        ...
        '''
        Reads the next message from the rosbag that matches the specified topics and read interval.
        ...
        while self.reader.has_next():  # Loop until there are no more messages
            topic, data, timestamp = self.reader.read_next()  # Read the next message
    ...
    def read_bag(
        ...
        '''
        Reads all messages from the rosbag that match the specified topics and interval.
        ...
        while True:
            ...
            self.msgs[topic].append((msg, timestamp))  # Store the message along with its timestamp
    ...
    def get_read_interval(
        ...
        '''
        Returns the read interval if it was set during initialization, or None if it was not set.
    ...
    def check_read_msg(self, timestamp: float, topic: str):
        '''
        Checks if a message should be read based on the read interval and the topics specified in the constructor.
    ...
    def check_msg_inside_interval(self, timestamp: float) -> bool:
        ...
        if self.timestamp_to_sec(timestamp) < first_message_time + self.read_interval[0] or self.timestamp_to_sec(timestamp) > first_message_time + self.read_interval[1]:
            return False

        return True

    @staticmethod
    def timestamp_to_sec(
        timestamp: float
    ):
        '''
