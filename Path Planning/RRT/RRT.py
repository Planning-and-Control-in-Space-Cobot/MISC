...
class RRTPlanner3D:
    ...
    def __init__(self, env: CollisionEnvironment3D, step_size=0.3, goal_tolerance=0.3,
                 goal_sample_rate=0.1, max_iters=1000):
        self.env = env
    ...
    def _sample(self, goal):
        if np.random.rand() < self.goal_sample_rate:
            return goal
        pos = np.random.uniform([0, 12.8, 0], [10, 13.2, 2])
    ...
    def run(self):
        start = State(np.array([-1.0, 12.0, 1.0]), np.zeros(3), np.array([0, 0, 0, 1]), np.zeros(3))
        goal = State(np.array([10.0, 7.0, 1.0]), np.zeros(3), np.array([0, 0, 0, 1]), np.zeros(3))
