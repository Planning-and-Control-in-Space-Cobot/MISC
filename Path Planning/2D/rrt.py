...
class PathPlanning:
    def __init__(self, map_obj, step_size=3, max_iterations=10000, goal_sample_rate=0.5, radius=5):
        ...
        for _ in range(self.max_iterations):
            ...
            if self._is_collision_free(nearest_node, new_node):
                ...
                for near_node in near_nodes:
                    cost = near_node.cost + np.hypot(new_node.x - near_node.x, new_node.y - near_node.y)
                ...
                node_list.append(new_node)
                explored_paths.append(((best_node.x, best_node.y), (new_node.x, new_node.y)))

                for near_node in near_nodes:
                    cost = new_node.cost + np.hypot(new_node.x - near_node.x, new_node.y - near_node.y)
                    if cost < near_node.cost and self._is_collision_free(new_node, near_node):
    ...
    def plot_map_with_paths(self, explored_paths, final_path):
        ...
        for path in explored_paths:
            plt.plot([path[0][0], path[1][0]], [path[0][1], path[1][1]], 'r-', linewidth=0.5)
        ...
        plt.scatter(self.map_obj.start[0], self.map_obj.start[1], color='b', marker='o', label='Start')
        plt.scatter(self.map_obj.goal[0], self.map_obj.goal[1], color='g', marker='o', label='Goal')
