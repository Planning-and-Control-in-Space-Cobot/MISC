...
class LongHorizonMPC():
    ...
    def setup_problem(self,
        ...
        for i in range(N-1):
            ...
            self.opti.subject_to(self.p[:, i+1] == self.p[:, i] + self.v[:, i] * self.dt)
            self.opti.subject_to(self.v[:, i+1] == self.v[:, i] + (R.as_matrix() @ F) / self.m * self.dt)

            self.opti.subject_to(self.q[:,i+1] == self.quaternion_integration(self.q[:, i], self.w[:, i], self.dt))
            self.opti.subject_to(self.w[:,i+1] == self.w[:, i] + ca.inv(self.J) @ (M - ca.cross(self.w[:, i], self.J @ self.w[:, i])) * self.dt) 
        ...
        for i in range(N):
            ...
            cost += 1 / 2 * ca.trace(ca.MX.eye(3) - R.as_matrix() @ R_des.as_matrix().T) * goal_cost
